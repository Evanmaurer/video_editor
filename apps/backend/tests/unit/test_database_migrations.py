...
from montage_backend.database_migrations import migrate_media_items_schema, migrate_project_db_schema
...
@pytest.mark.asyncio
async def test_migrate_project_db_creates_metadata_table(tmp_path: Path) -> None:
    ...
    async with engine.begin() as conn:
        ...
        await conn.run_sync(migrate_project_db_schema)

    async with engine.connect() as conn:
        table = (
            await conn.execute(
                text(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='media_metadata_features'",
                ),
            )
        ).fetchone()
