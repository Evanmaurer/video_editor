...
async def test_albion_analysis_engine_runs_registered_detectors():
    ...
    assert result.summary.detector_count == 8
    ...
    assert "combat" in result.detector_results
    assert "bomb" in result.detector_results
    assert "engagement" in result.detector_results
    assert "highlight" in result.detector_results
...
def test_albion_analyzer_cache_key_includes_detector_versions():
    ...
    assert "combat" in key
    assert "bomb" in key
    assert "engagement" in key
    assert "highlight" in key
