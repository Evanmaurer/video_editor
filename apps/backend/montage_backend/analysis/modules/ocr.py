...
class OcrAnalyzer(Analyzer):
    module_id = AnalysisModuleId.OCR
    version = "ocr-analyzer-v1.0"
    ...
    async def analyze(
        ...
        *,
        ...
        await ctx.report(0.0, "Probing video for OCR")
        ctx.check_cancelled()
        ...
        for index, timestamp_ms in enumerate(timestamps):
            ...
            await ctx.report(progress, f"OCR frame {index + 1}/{total}")
            ...
            raw_detections = await asyncio.to_thread(engine.recognize_png, png_bytes)
        ...
        await ctx.report(1.0, f"OCR found {len(result.unique_texts)} unique texts")
        confidence = 0.5 if engine.engine_id == "unavailable" else 0.8
        if result.summary.detection_count > 0:
            confidence = min(0.95, 0.7 + result.summary.unique_text_count * 0.02)
        ...
        return AnalysisOutput(
            ...
            reasoning=(
                ...
                f"{result.summary.unique_text_count} unique"
            ),
        )
    ...
    def _get_engine(self) -> OcrEngine:
        ...
        if self._resolved_engine is None:
            self._resolved_engine = resolve_ocr_engine(prefer_gpu=self._prefer_gpu)
